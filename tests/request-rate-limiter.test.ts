...
import { timingSafeStrEqual } from '../src/utils/timing-safe.js';
...
describe('rateLimiterFromEnv', () => {
  it('honours LUCKAGENT_RATE_LIMIT_DISABLED=1', () => {
    const rl = rateLimiterFromEnv({ LUCKAGENT_RATE_LIMIT_DISABLED: '1' } as NodeJS.ProcessEnv);
    for (let i = 0; i < 1000; i++) expect(rl.check('ip')).toBeNull();
  });

  it('reads LUCKAGENT_RATE_LIMIT_MAX and AUTH_FAILS', () => {
    const rl = rateLimiterFromEnv({
      LUCKAGENT_RATE_LIMIT_MAX: '2',
      LUCKAGENT_RATE_LIMIT_AUTH_FAILS: '2',
  ...
  it('falls back to defaults on invalid env values', () => {
    const rl = rateLimiterFromEnv({ LUCKAGENT_RATE_LIMIT_MAX: 'nonsense' } as NodeJS.ProcessEnv);

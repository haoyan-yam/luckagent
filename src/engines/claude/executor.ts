 ...
 *
 * Add a var here when you need Luckagent users to be able to enable a
...
function createSpawnFn(authEnv?: Record<string, string>): (options: SpawnOptions) => SpawnedProcess {
  ...
  //   (a) LUCKAGENT_PREFER_ENV_AUTH=true (explicit opt-in flag), or
  ...
  const preferEnvAuth =
    process.env.LUCKAGENT_PREFER_ENV_AUTH === 'true' ||
    !!(
  ...
  return (options: SpawnOptions): SpawnedProcess => {
    ...
    // setting CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=0 in Luckagent's parent env.
    ...
    // CLAUDE_CODE_DISABLE_AUTO_MEMORY=1 in Luckagent's parent env.
 ...
 *     (Luckagent's spawn handler merges `queryOptions.env` on top of
...
export class ClaudeExecutor {
  ...
  private buildQueryOptions(cwd: string, sessionId: string | undefined, abortController: AbortController, outputsDir?: string, apiContext?: ApiContext): Record<string, unknown> {
    ...
    if (apiContext) {
      ...
      // Port and secret are already set as LUCKAGENT_* env vars in config.ts.
      appendSections.push(
        `## Luckagent API\nYou are running as bot "${apiContext.botName}" in chat "${apiContext.chatId}".\nUse the /luckagent skill for full API documentation (agent bus, scheduling, bot management).`
      );
      ...
      appendSections.push(
        [
          ...
          `When the user asks you to create an agent team, ALWAYS prefix the team name with \`${teamNs}-\` to avoid collisions with other Luckagent chats sharing this machine. For example: \`${teamNs}-research\`, \`${teamNs}-refactor\`.`,
          'Display mode is forced to `in-process` (no tmux/iTerm2 in Luckagent). Teammates show up in the user\'s Feishu card via TeammateIdle / TaskCreated / TaskCompleted events — you don\'t need to walk the user through Shift+Down navigation.',
      ...
      if (apiContext.groupMembers && apiContext.groupMembers.length > 0) {
        ...
        if (groupId) {
          appendSections.push(
            `## Group Chat\nYou are in a group chat (group: ${groupId}) with these bots: ${others.join(', ')}.\nTo talk to another bot, use: \`luckagent talk <botName> grouptalk-${groupId}-<botName> "message"\`\nExample: \`luckagent talk ${others[0]} grouptalk-${groupId}-${others[0]} "hello"\`\nIMPORTANT: Always use the grouptalk-${groupId}-<botName> chatId pattern when talking to other bots in this group.`
          );
        } else {
          appendSections.push(
            `## Group Chat\nYou are in a group chat with these bots: ${others.join(', ')}.\nUse \`luckagent talk <botName> <chatId> "message"\` to communicate with other bots in the group.`
          );
        }
      }
    }
  ...
  startExecution(options: ExecutorOptions): ExecutionHandle {
    ...
    const memoryGuardHook = createMemoryGuardHook(this.logger); // [design-note U]
    queryOptions.hooks = {
      PreToolUse: [
        {

...
export function createHookBridge(options?: HookBridgeOptions): PtyHookBridge {
  ...
  const bridgeDir = join(tmpdir(), `luckagent-pty-${bridgeId}`);
  ...
  const guardScriptPath = join(bridgeDir, 'memory-guard.cjs'); // [design-note U]
  ...
  async function writeSettings(): Promise<string> {
    ...
    // [design-note U] 记忆库出站闸门：PTY 后端的 claude 只认 --settings 里的 command 钩子，

...
function createSpawnFn(explicitApiKey?: string): (options: SpawnOptions) => SpawnedProcess {
  ...
  return (options: SpawnOptions): SpawnedProcess => {
    ...
    // here so Luckagent stays on this default even if Claude Code flips its
...
export interface PersistentExecutorOptions {
  ...
  logger: Logger;
  /**
   * Luckagent bot/chat context. Stable for the lifetime of the executor
...
export class PersistentClaudeExecutor extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    // System prompt: bake in Luckagent context + outputs dir + team namespace

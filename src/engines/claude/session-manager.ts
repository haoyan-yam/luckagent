...
// session files (~/.luckagent/<bot>/sessions-*.json, sessions.db).
...
export class SessionManager {
  ...
  constructor(
    ...
    const dataDir = process.env.SESSION_STORE_DIR
      || path.join(os.homedir(), '.luckagent');
  ...
  setGoal(chatId: string, condition: string | undefined): void {
    ...
    if (condition) {
      ...
      const maxIterations = Number(process.env.LUCKAGENT_CODEX_GOAL_MAX_ITERATIONS);

 ...
 *
 *   GET /api/executors
 *     → snapshot of every persistent Claude process Luckagent is currently
 ...
 * registry is lazy-init — first ever turn after Luckagent start populates it).
 */
export async function handleExecutorRoutes(
  ...
  jsonResponse(res, 200, {
    persistentExecutorEnabled: resolvePersistentExecutorEnvDefault(
      process.env.LUCKAGENT_PERSISTENT_EXECUTOR,
    ),
    count: out.length,
    executors: out,
  });

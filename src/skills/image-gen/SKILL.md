...
**主体 + 场景明确的请求直接出图，不要提问**——"一张外滩夜景""草地上的柴犬""赛博朋克风格的上海街头"都属于此类：用默认参数（medium 质量、方图或按内容选比例、不透明底）立刻生成，快速给结果比追问偏好更有价值；用户不满意自然会提修改要求（那才是澄清的最佳时机）。
...
1. **用途**：海报 / icon / banner / 产品图 / 插画（决定 size 比例与透明底）
2. **质量档**：low（试稿）/ medium（默认）/ high（正式交付）
3. **画面文字**：要哪些字（不写 = 无文字）
4. **风格 / 参考图**：有参考图就传 `--image`（不选 = 按内容自定）

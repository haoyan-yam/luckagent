...
Authoritative parameter reference for the OpenAI Images API as used by this skill's `scripts/gen_image.py`.

## Scope
- GPT Image models: `gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, and `gpt-image-1-mini`.
...
| `gpt-image-1.5` | `low`, `medium`, `high`, `auto` | `low`, `high` | `1024x1024`, `1024x1536`, `1536x1024`, `auto` | True transparent-background output (`background=transparent`) and backward-compatible workflows |
...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter — passing it returns a 400 error. Two supported paths (see `SKILL.md`):

- **Path A (native transparency)**: `--model gpt-image-1.5 --background transparent` with a transparent-capable output format (`png` or `webp`). Best for simple subjects/icons, and required for complex edges (hair, fur, glass, smoke, translucency).
- **Path B (gpt-image-2 quality)**: generate on a flat chroma-key background, then extract alpha locally with `python3 scripts/remove_chroma_key.py` (in this skill's `scripts/` directory).

## Output
- `data[]` list with `b64_json` per image
- The bundled `scripts/gen_image.py` CLI decodes `b64_json` and writes output files for you.
...
- If a request fails because a specific option is unsupported by the selected GPT Image model, retry without that option only when the option is not required by the user. If transparent output is required with `gpt-image-2`, switch to `gpt-image-1.5` or the chroma-key path instead of dropping `background=transparent`.

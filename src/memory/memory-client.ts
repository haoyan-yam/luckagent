...
/**
 * Talks to the central `luckagent-core` service over HTTP(S) (default
 * `http://localhost:9200` — set LUCKAGENT_CORE_URL for a remote/self-hosted
 * host). All endpoints live under `/api/memory/*` and require a
 * `Authorization: Bearer <token>` header.
 *
 * Token resolution order:
 *   1. constructor `tokenOverride`
 *   2. `LUCKAGENT_CORE_TOKEN` env var
 *   3. first non-empty line of `~/.luckagent-core/token`
 *
 * Base URL resolution order:
 *   1. constructor `baseUrlOverride`
 *   2. `LUCKAGENT_CORE_URL` env var
...
export class MemoryClient {
  /** Base URL for luckagent-core (no trailing slash). Public so callers
  ...
  constructor(
    ...
    const resolved = tokenOverride
      || process.env.LUCKAGENT_CORE_TOKEN
      || loadTokenFromFile();
    this.token = resolved || '';
    ...
    if (!this.token) {
      this.logger.warn(
        ...
        'MemoryClient: no luckagent-core token found (set LUCKAGENT_CORE_TOKEN or write ~/.luckagent-core/token). Requests will fail with 401.',
      );
    }
  }
  ...
  private async request<T>(path: string, options?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`luckagent-core ${res.status}: ${body}`);
    }
  ...
  async health(): Promise<HealthStatus> {
    ...
    if (!res.ok) {
      throw new Error(`luckagent-core health ${res.status}`);
    }
  ...
  /**
   * Fetch full document content by id or absolute path. luckagent-core accepts
  ...
  private unwrapArray<T>(data: unknown, key: string): T[] {
    ...
    this.logger.warn({ responseType: typeof data, key }, 'Unexpected array response format from luckagent-core');
    return [];
  }
  ...
  private unwrapSingle<T>(data: unknown, key: string): T {
    ...
    this.logger.warn({ responseType: typeof data, key }, 'Unexpected single-object response format from luckagent-core');

 ...
 * `LUCKAGENT_PERSISTENT_EXECUTOR=false` (or `=0`) in the env.
 *
...
export class MessageBridge {
  ...
  constructor(
    ...
    // [design-note P] 出站台账（引用回复解析用）；可选，未传时引用解析走 API/降级
  ...
  /** Inject Luckagent Agent Teams store so cards can show team and run state. */
   ...
   *   3. otherwise: LUCKAGENT_PERSISTENT_EXECUTOR=false (or '0') env → off
  ...
  private isPersistentExecutorEnabled(): boolean {
    ...
    return resolvePersistentExecutorEnvDefault(process.env.LUCKAGENT_PERSISTENT_EXECUTOR);
  }
  ...
  private getOrCreateRegistry(): ExecutorRegistry {
    if (!this.persistentRegistry) {
      ...
      const cfg = this.config.persistentExecutor;
      const idleEnv = Number(process.env.LUCKAGENT_PERSISTENT_EXECUTOR_IDLE_MS);
      const maxEnv = Number(process.env.LUCKAGENT_PERSISTENT_EXECUTOR_MAX_CONCURRENT);
  ...
  /**
   * [design-note F] 新的 AskUserQuestion 选择卡出现时，@【本任务发起人】触发飞书强提醒，
  ...
  private async handleBetweenTurnQuestion(
    ...
    if (!cardMessageId) {
      ...
      try {
        ...
        if (cardMessageId) void this.atRequesterForQuestion(chatId); // [design-note F] @ 提问人
      } catch (err) {
  ...
  private async flushSpontaneous(chatId: string): Promise<void> {
    ...
    // [design-note Q] spontaneous 活动常伴随后台任务(如慢速生图)把产物 cp 进
    ...
    if (buf.snippets.length === 0) {
      ...
      await sweepOutputs(); // [design-note Q]
      return;
    }
    ...
    await sweepOutputs(); // [design-note Q] 卡片先行,产物紧随
  }
  ...
  async handleMessage(msg: IncomingMessage): Promise<void> {
    ...
    if (this.isBusy(chatId)) {
      ...
      // [design-note P] 追加 parentId 相等判断：引用不同消息的排队消息不合并（各注入各的引文）
  ...
  private async executeQueryInner(msg: IncomingMessage): Promise<void> {
    ...
    // [design-note N] 下载失败绝不静默：prompt 里写明文件名与原因，并要求 agent 告知用户。
    ...
    let extraMediaFailures = 0; // [design-note N]
    ...
    // [design-note P] 引用回复上下文注入：自家消息走出站台账（卡片记终版文本、
    // 媒体记 key，本地被design-note E 清掉后可回捞），他人消息走 message.get；被引媒体落地为
    ...
    // [design-note H] 把「本条消息是谁发的」注入本回合 prompt —— 桥接原本
    ...
    // from `luckagent restart/update`), prepend a system-reminder to this chat's
    ...
    if (shouldRemindRestart(chatId)) {
      ...
      prompt =
        `<system-reminder>\n` +
        `Luckagent bridge 已于约 ${secs} 秒前被重启过（很可能是上一轮你自己执行了 luckagent restart/update —— 进程已重生、会话已恢复）。` +
        `重启已经完成，请勿再次执行 luckagent restart 或 luckagent update。` +
    ...
    // [design-note T] 新会话首回合：交接块放最顶上（在提问人/引用/重启提醒之前）。
    ...
    try {
        ...
        if (state.status === 'waiting_for_input' && state.pendingQuestion) {
          ...
          if (runningTask.questionCardMessageId && this.sender.updateQuestionCard) {
            ...
            if (qMsgId) {
              ...
              void this.atRequesterForQuestion(chatId); // [design-note F] @ 提问人
            } else {
      ...
      metrics.incCounter('luckagent_tasks_total');
      metrics.incCounter('luckagent_tasks_by_status', lastState.status === 'complete' ? 'success' : 'error');
      metrics.observeHistogram('luckagent_task_duration_seconds', durationMs / 1000);
      if (lastState.costUsd) metrics.observeHistogram('luckagent_task_cost_usd', lastState.costUsd);
      ...
      await sendCompletionNotice({
        ...
        state: lastState,
        durationMs,
        // [design-note] 仅群聊:完成通知作为「引用回复」发到触发消息上,让提问人
      ...
      if ((isStaleSessionError(errMsg) || isContextOverflowError(errMsg)) && session.sessionId) {
        ...
        try {
          ...
          await sendCompletionNotice({
            ...
            state: lastState,
            durationMs,
            // [design-note] 仅群聊:完成通知作为引用回复,通知落在答案就绪时(见另一处同款注释)
            replyToMessageId: chatType === 'group' ? msgId : undefined,
          });
          await this.outputHandler.sendOutputFiles(chatId, outputsDir, processor, lastState);
      ...
      metrics.incCounter('luckagent_tasks_total');
      metrics.incCounter('luckagent_tasks_by_status', 'error');
  ...
  async executeApiTask(options: ApiTaskOptions): Promise<ApiTaskResult> {
    ...
    try {
      ...
      this.costTracker.record({ botName: this.config.name, userId, success: lastState.status === 'complete', costUsd: lastState.costUsd, durationMs });
      metrics.incCounter('luckagent_api_tasks_total');
      metrics.observeHistogram('luckagent_task_duration_seconds', durationMs / 1000);
      if (lastState.costUsd) metrics.observeHistogram('luckagent_task_cost_usd', lastState.costUsd);

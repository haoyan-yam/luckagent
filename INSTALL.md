...
| 东西 | 说明 |
| --- | --- |
| 安装包 | `luckagent-installer-v0.2.3.tar.gz`（+ 同名 `.sha256` 校验文件） |
...
- **隔空投送（AirDrop）**：从旧机器把 `luckagent-installer-v0.2.3.tar.gz` 投过去（默认落在 `~/Downloads`）
- **U 盘**：拷贝到 U 盘再拷进 `~/Downloads`
- **scp**（两台机器同一局域网时）：

```bash
scp luckagent-installer-v0.2.3.tar.gz 用户名@mac-mini.local:~/Downloads/
```
...
```bash
cd ~/Downloads
shasum -a 256 -c luckagent-installer-v0.2.3.tar.gz.sha256
```
...
```bash
cd ~/Downloads
tar -xzf luckagent-installer-v0.2.3.tar.gz

 ...
 *
 * Auth flow:
 *   - LUCKAGENT_CORE_URL (default http://localhost:9200) + LUCKAGENT_CORE_ADMIN_TOKEN (env or --token)
 *   - All requests go through the admin HTTP endpoints, so the CLI works
 *     equally well against a remote luckagent-core server.
 */
...
Env:
  LUCKAGENT_CORE_URL          base URL (default http://localhost:9200)
  LUCKAGENT_CORE_ADMIN_TOKEN  admin bearer token
...
async function run() {
  ...
  const base = (args.flags.url as string) || process.env.LUCKAGENT_CORE_URL || 'http://localhost:9200';
  const token = (args.flags.token as string) || process.env.LUCKAGENT_CORE_ADMIN_TOKEN || '';
  if (!token) {
    console.error('error: LUCKAGENT_CORE_ADMIN_TOKEN env or --token <token> required');
    process.exit(2);
  }

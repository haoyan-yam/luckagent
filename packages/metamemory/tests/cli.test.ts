...
describe('loadConfig', () => {
  ...
  it('uses env vars when set', () => {
    const cfg = loadConfig({ LUCKAGENT_CORE_URL: 'http://example/', LUCKAGENT_CORE_TOKEN: 'tok1' });
    expect(cfg.url).toBe('http://example');
    expect(cfg.token).toBe('tok1');
  });

  it('defaults URL and reads token from ~/.luckagent-core/token', () => {
    fs.mkdirSync(path.join(tmpHome, '.luckagent-core'));
    fs.writeFileSync(path.join(tmpHome, '.luckagent-core', 'token'), 'file-tok\n');
...
describe('cmdCreate / cmdMkdir — write target', () => {
  ...
  it('create: agent-kind member → /users/<owner>/agents/<bot>', async () => {
    // agent-kind: botName !== ownerName (bot token issued via `luckagent agents create`)
...
describe('defaultWritePrefix — pure helper', () => {
  ...
  it('member, agent-kind (botName !== ownerName) → /users/<owner>/agents/<bot>, regardless of memoryPublic', () => {
    // Bot token issued via `luckagent agents create`
    expect(defaultWritePrefix({

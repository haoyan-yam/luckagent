...
import type { ParsedArgs } from '@luckagent/cli-core/args';

export { parseArgs } from '@luckagent/cli-core/args';
export type { ParsedArgs } from '@luckagent/cli-core/args';
...
/**
 * `luckagent memory share <doc_id> [on|off]` — toggle a single document's
...
/**
 * `luckagent memory visibility [public|private]` — read or toggle the calling
...
export function printHelp(): void {
  process.stdout.write(
    `luckagent memory — luckagent-core memory CLI

Usage: luckagent memory <command> [args]
...
Sharing (read access):
  ...
  bot's visibility ('luckagent memory visibility public|private'). Override per
  doc with --share / --no-share on create/update, or flip an existing doc with
  'luckagent memory share <doc_id> on|off'. A shared doc is readable by any

...
- **生产可靠优先。** 本项目源自一套在真实团队里连续运行数月的飞书 bot 集群，消息合并、附件收发、发送目录清理、引用上下文注入、空闲会话自动换新、记忆库出站闸门等 18 项行为增强都是实际踩坑后的沉淀（见[设计笔记](docs/design-notes.md)），并配有近千条自动化测试。
...
| [引擎配置](docs/engines.md) | Claude / DeepSeek / MiniMax 各引擎的认证路线与切换方式 |
